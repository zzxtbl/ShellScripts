...
#version:v1.5
####################系统下获取有关硬件方面的信息################################
#install_dmidecode  Linux 系统下获取有关硬件方面的信息
...
useradd zzxtbl
echo "oldboy_zkq"|passwd --stdin zzxtbl
cp /etc/sudoers /etc/sudoers.ori
echo "zzxtbl ALL=(ALL) ALL" >>/etc/sudoers
check
...
chkconfig sysstat on  #sysstat是一个软件包，包含监测系统性能及效率的一组工具
check
...
check
#########################clientmqueue中存放cron中执行的程序有输出内容#####################################
...
#########################软件apache,nginx,mysql...web 安装编写的脚本#####################################
